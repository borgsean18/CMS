<?php
/**
 * Plugin Name: shadowfiend: Large Blog Module
 ...
 */
add_action('widgets_init', 'shadowfiend_register_large_blog_module');

function shadowfiend_register_large_blog_module(){
	register_widget('shadowfiend_large_blog');
}
 ...
 *
 */ 
class shadowfiend_large_blog extends WP_Widget {
	...
	function __construct(){
		/* Widget settings. */	
		$widget_ops = array('classname' => 'module-large-blog', 'description' => esc_html__('[Content module] Displays latest posts in large blog layout in content section.','zalora'));
		
		/* Create the widget. */
		parent::__construct('shadowfiend_large_blog', esc_html__('*shadowfiend: Module Large Blog','zalora'), $widget_ops);
	}
	...
	function widget($args, $instance){	
        ...
        <div id="<?php echo esc_attr($uid);?>" class="shadowfiend-large-blog-wrapper" >
         	
    		<div class="large-blog-content-container">
                <?php while ( $query -> have_posts() ) : $query -> the_post(); ?>
                    <?php $post_id = get_the_ID();?>
                    <?php echo zalora_large_blog_render($post_id, $excerpt_length);?>
                <?php endwhile;?>
            </div>	
